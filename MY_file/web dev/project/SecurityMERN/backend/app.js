...
app.get('/search/:key',async(req,res)=>{
    let searchproduct=await Product.find({
        //show if any one match .we can also search with one value also
        "$or":[
...
//posting  studentdetails 
app.post("/student",async(req,res)=>{
    const {name ,studentId,branch}=req.body


    const studentdetails=new Student({name ,studentId,branch});
    await studentdetails.save();
      res.status(201).send(req.body);
});
//getting student 
app.get("/student/:studentId",async(req,res)=>{
    const studentavail=await Student.findOne({studentId:req.params.studentId});
    console.log(req.params.studentId)
    if(studentavail){
        res.status(201).send(studentavail);
    }
    else{
